...
const App: React.FC = () => {
  ...
  const filteredProperties = useMemo(() => {
    return MOCK_PROPERTIES.filter(prop => {
      ...
      // Type Filter
      const matchesType = filters.type === 'All Types' || prop.type === filters.type;
      ...
      if (filters.budget !== 'Any Budget') {
        ...
        if (filters.budget === 'luxury') {
          ...
          matchesBudget = isNgn ? (price >= 100000000 && price < 200000000) : (price >= 500000 && price < 1000000);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main>
        ...
        <section className="py-20 bg-slate-50">
          <div className="container mx-auto px-4 md:px-8">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { label: 'Properties in Portfolio', val: currency === 'NGN' ? '₦12B+' : '$7.5M+' },
                { label: 'Nigerian Cities', val: '12' },
                { label: 'VVIP Clients', val: '450+' },
                { label: 'Success Rate', val: '98%' }
              ].map((stat, i) => (
                <div key={i} className="text-center p-8 bg-white rounded-3xl shadow-sm border border-slate-100 group hover:border-amber-400 transition-all duration-300">
                  <div className="text-4xl font-black text-[#1A2B4C] mb-2 group-hover:scale-110 transition-transform">{stat.val}</div>
          ...
          <div className="container mx-auto px-4 md:px-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
              <div className="max-w-xl">
                <span className="text-amber-500 font-black uppercase tracking-[0.3em] text-xs mb-3 block">Exclusive Listings</span>
                <h2 className="text-5xl font-black text-[#1A2B4C] mb-6">Our Premium Properties</h2>
                <div className="flex flex-wrap gap-2">
                  {filters.location && <span className="bg-slate-100 px-3 py-1 rounded-full text-xs font-bold text-slate-600">"{filters.location}"</span>}
                  {filters.type !== 'All Types' && <span className="bg-slate-100 px-3 py-1 rounded-full text-xs font-bold text-slate-600">{filters.type}</span>}
                  {(filters.location || filters.type !== 'All Types' || filters.budget !== 'Any Budget') && (
                    <button 
                      onClick={() => setFilters({ location: '', type: 'All Types', budget: 'Any Budget' })}
              ...
              <div className="flex items-center gap-6">
                 <div className="text-right hidden md:block">
                   <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Showing</p>
                   <p className="text-2xl font-black text-[#1A2B4C]">{filteredProperties.length} Results</p>
                 </div>
            ...
            {filteredProperties.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredProperties.map(prop => (
                  <PropertyCard key={prop.id} property={prop} displayCurrency={currency} />
                ))}
              </div>
            ) : (
              <div className="py-32 flex flex-col items-center justify-center text-center max-w-md mx-auto">
                ...
                <h3 className="text-3xl font-bold text-[#1A2B4C] mb-4">No Properties Found</h3>
                <p className="text-slate-500 mb-8">We couldn't find any properties matching your current filters. Try adjusting your search criteria or clear filters to see all listings.</p>
                <button 
                  onClick={() => setFilters({ location: '', type: 'All Types', budget: 'Any Budget' })}
        ...
        <section id="services" className="py-32 bg-[#1A2B4C] text-white overflow-hidden relative">
          ...
          <div className="container mx-auto px-4 md:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
              <div>
                <span className="text-amber-400 font-black uppercase tracking-[0.3em] text-xs mb-4 block">The AddyHolly Way</span>
                <h2 className="text-6xl font-black mb-10 leading-tight">Elevating Nigerian Real Estate</h2>
                <p className="text-slate-300 mb-12 text-xl leading-relaxed font-light">
                  We specialize in the luxury segment of the Nigerian market, providing bespoke property solutions for high-net-worth individuals and corporate entities.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  {[
                    { title: 'Market Integrity', desc: 'Transparent dealings across all Lagos & Abuja assets.' },
                    { title: 'Global Network', desc: 'Connecting Nigerian elite to international investments.' },
                    { title: 'Expert Valuation', desc: 'Precise appraisals in a dynamic economic climate.' },
                    { title: 'Legal Security', desc: 'Watertight documentation for peace of mind.' }
                  ].map((item, i) => (
              ...
              <div className="relative">
                ...
                <div className="absolute -bottom-12 -left-12 bg-amber-400 text-[#1A2B4C] p-10 rounded-[40px] shadow-2xl hidden md:block">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest opacity-80">Years of Luxury<br/>Experience</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="container mx-auto px-4 md:px-8">
            <div className="bg-slate-900 rounded-[60px] p-12 md:p-24 text-center relative overflow-hidden shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)]">
              ...
              <div className="relative z-10">
                <h2 className="text-5xl md:text-7xl font-black text-white mb-10">Own Your Piece of<br/><span className="text-amber-400">Paradise</span></h2>
                <p className="text-slate-400 text-xl mb-16 max-w-2xl mx-auto font-light leading-relaxed">
                  The Nigerian property market moves fast. Partner with AddyHolly to ensure you never miss an elite opportunity.
                </p>
